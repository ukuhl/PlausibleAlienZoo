...
Similar to the modeling of \textit{closest \glspl{CFE}} in~\refeq{eq:counterfactual_opt_original}, different realizations of \textit{computational plausibility} have been proposed~\citep{looveren_interpretable_2019,poyiadzi_face_2019,artelt_convex_2020}. 
...
We developed a game-like experimental design, the Alien Zoo framework~\citep{IAZ ARXIV}.
...
After performing three pilots with 10 users each to refine the experimental design, we recruited a total of 100 participants for final assessment. % Batch info neccesary? % report that users were excluded if the done it before?
