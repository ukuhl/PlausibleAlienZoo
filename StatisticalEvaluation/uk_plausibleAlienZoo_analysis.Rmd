...
rt_source = list.files(path = "../UserData", pattern="reactionTime.csv",full.names=TRUE)
...
Quick sanity check: after cleaning, we have `r length(unique(df_perf$userId))` participants.
...
H1.2_p_RTPerTrial_summary_anno <- ggplot(df_RTPerTrial_summary, aes(x=as.factor(TrialNr), y=mean/1000, group = group, color= group))+ 
  ...
  theme_bw(base_size = 10)+
  scale_y_continuous(limits = c(8800/1000, 37000/1000))+
  scale_x_discrete(breaks=1:numTrials)+
  scale_colour_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  scale_fill_manual("Group", values=c(Ccol,Pcol), labels = c("Closest", "Plausible"))+
  theme(legend.position="bottom")+
  ...
  geom_segment(aes(x = 3, y = 20000/1000, xend = 3, yend = 22000/1000),colour = "black", size=0.2) +
  geom_segment(aes(x = 4, y = 20000/1000, xend = 4, yend = 22000/1000),colour = "black", size=0.2) +
  geom_segment(aes(x = 3, y = 22000/1000, xend = 4, yend = 22000/1000),colour = "black", size=0.2) +
  geom_text(x = 3.5,  y = 22500/1000, label = "*", colour = "black")+
  # THIRD EFFECT: trial 4 vs. trial 5 (p=0.0476) 
  geom_segment(aes(x = 4, y = 25000/1000, xend = 4, yend = 27000/1000),colour = "black", size=0.2) +
  geom_segment(aes(x = 5, y = 25000/1000, xend = 5, yend = 27000/1000),colour = "black", size=0.2) +
  geom_segment(aes(x = 4, y = 27000/1000, xend = 5, yend = 27000/1000),colour = "black", size=0.2) +
  geom_text(x = 4.5,  y = 27500/1000, label = "*", colour = "black")
...
# current setting: plants 2, and plants 4 or 5 were relevant:
...
H1.3_figure2_UserResponseMatches = ggplot(df_survey_relevantPlants_plot, aes(x=group, y=mean, fill=group)) + 
  ...
  geom_hline(aes(yintercept=2.5, linetype="Mean no.\nof matches for\n random choice"), color = "black", size=0.5)+
...
* Is there a significant difference in terms of matches between plants judged as relevant and ground truth?: We compared number of matches for users in plausible condition (M = `r df_survey_relevantPlants_summary$mean[df_survey_relevantPlants_summary$group=="P"]`, SEM = `r df_survey_relevantPlants_summary$sem[df_survey_relevantPlants_summary$group=="P"]`) and users in the closest condition (M = `r df_survey_relevantPlants_summary$mean[df_survey_relevantPlants_summary$group=="C"]`, SEM = `r df_survey_relevantPlants_summary$sem[df_survey_relevantPlants_summary$group=="C"]`) using a `r item1t` test. This showed

  + for wilcoxon test: U=`r item1test$statistic `, p=`r item1test$p.value `, r = `r item1effsize `

* Is there a significant difference in terms of matches between plants judged as irrelevant and ground truth?: We compared number of matches for users in plausible condition (M = `r df_survey_irrelevantPlants_summary$mean[df_survey_irrelevantPlants_summary$group=="P"]`, SEM = `r df_survey_irrelevantPlants_summary$sem[df_survey_irrelevantPlants_summary$group=="P"]`) and users in the closest condition (M = `r df_survey_irrelevantPlants_summary$mean[df_survey_irrelevantPlants_summary$group=="C"]`, SEM = `r df_survey_irrelevantPlants_summary$sem[df_survey_irrelevantPlants_summary$group=="C"]`) using a `r item2t` test. This showed

  + for wilcoxon test: U=`r item2test$statistic `, p=`r item2test$p.value `, r = `r item2effsize `

## Add on: Did number of matches differ significantly from expected random?

```{r echo=FALSE, fig.height = 3, fig.width = 5, fig.align = "center"}

# add on: run a test whether groups are signifcantly better than random

# first: test for normality of distributions using shapiro test
# if p-value is lower than 0.05, you can conclude that the sample deviates from normality
# --> ALL HIGHLY NON-NORMAL!
# ergo, run a Wilcoxon one-sample signed rank test
## note: this actuallty tests for the median, but can also be used for the mean, cause:
## "The signed rank test is a test for the pseudo-median. When accompanied by the assumption of symmetry (which the signed rank test needs for the permutations to be equally likely under the null), the population pseudo-median will equal the population median, and (by symmetry) will also equal the population mean when it is finite (which is hereafter assumed to save repeating it every time I mention the mean). Which is to say, under the usual assumptions the signed rank test is a suitable test for a mean as well as a median. (https://stats.stackexchange.com/questions/239293/non-parametric-altervative-to-one-sample-t-test)"

## relevant plants, plausible group:
shapiro=shapiro.test(df_survey_relevantPlants_wide$matches[df_survey_relevantPlants_wide$group=="P"])
wos1_test=wilcox.test(df_survey_relevantPlants_wide$matches[df_survey_relevantPlants_wide$group=="P"], mu = 2.5, alternative = "two.sided")
wos1_effsize=wos1_test$z_val/(sqrt(length(df_survey_relevantPlants_wide$matches[df_survey_relevantPlants_wide$group=="P"])))

## relevant plants, closest group:
shapiro=shapiro.test(df_survey_relevantPlants_wide$matches[df_survey_relevantPlants_wide$group=="C"])
wos2_test=wilcox.test(df_survey_relevantPlants_wide$matches[df_survey_relevantPlants_wide$group=="C"], mu = 2.5, alternative = "two.sided")
wos2_effsize=wos2_test$z_val/(sqrt(length(df_survey_relevantPlants_wide$matches[df_survey_relevantPlants_wide$group=="C"])))

## irrelevant plants, plausible group:
shapiro=shapiro.test(df_survey_irrelevantPlants_wide$matches[df_survey_irrelevantPlants_wide$group=="P"])
wos3_test=wilcox.test(df_survey_irrelevantPlants_wide$matches[df_survey_irrelevantPlants_wide$group=="P"], mu = 2.5, alternative = "two.sided")
wos3_effsize=wos3_test$z_val/(sqrt(length(df_survey_irrelevantPlants_wide$matches[df_survey_irrelevantPlants_wide$group=="P"])))

## irrelevant plants, closest group:
shapiro=shapiro.test(df_survey_irrelevantPlants_wide$matches[df_survey_irrelevantPlants_wide$group=="C"])
wos4_test=wilcox.test(df_survey_irrelevantPlants_wide$matches[df_survey_irrelevantPlants_wide$group=="C"], mu = 2.5, alternative = "two.sided")
wos4_effsize=wos4_test$z_val/(sqrt(length(df_survey_irrelevantPlants_wide$matches[df_survey_irrelevantPlants_wide$group=="C"])))

```

This post-hoc analysis revealed:

* relevant plants, plausible group: one-sample Wilcoxon signed rank test, W = `r wos1_test$statistic`, p = `r wos1_test$p.value*4` (bonf corrected), r = `r wos1_effsize`

* relevant plants, closest group: one-sample Wilcoxon signed rank test, W = `r wos2_test$statistic`, p = `r wos2_test$p.value*4` (bonf corrected), r = `r wos2_effsize`

* irrelevant plants, plausible group: one-sample Wilcoxon signed rank test, W = `r wos3_test$statistic`, p = `r wos3_test$p.value*4` (bonf corrected), r = `r wos3_effsize`

* irrelevant plants, closest group: one-sample Wilcoxon signed rank test, W = `r wos4_test$statistic`, p = `r wos4_test$p.value*4` (bonf corrected), r = `r wos4_effsize`
...
H1.3_figure2_UserResponseMatches$labels$title=c()
H1.3_figure2_UserResponseMatches$labels$x=c()
...
H1.3tmp <- H1.3_figure2_UserResponseMatches+theme(legend.title = element_blank(),legend.margin=margin(t = -0.25, unit='cm'))

# annotate statistical difference from random:
dat_anno <- data.frame(
  label = c("**", "**", "*"),
  group = c("P","C","P"),
  itemNo   = c(1, 2, 2),
...
# save
ggsave("Figures/H1_p_ShubsPerTrial_RTPerTrial_PAZ_FINAL.pdf",width = 8,height = 3.5) #width = 7, height = 2,
```
...
p1=H2.1_figure2_UserResponses_stackedBar+theme(title=element_blank(),
...
p1_T=H2.1_figure2_UserResponses_stackedBar+facet_wrap(vars(itemNo),nrow = 1, ncol = 2,labeller=as_labeller(c(`5`="\"Feedback helped me.\"",`6`="\"I could use the feedback.\"")))+
  theme(panel.spacing = unit(1, "lines"),axis.title.x=element_blank())

p1_T$labels$title=c()

H2.2_figure2_UserResponses_stackedBar$labels$title=c()
p2=H2.2_figure2_UserResponses_stackedBar+theme(axis.title.y=element_blank())

p2_T=H2.2_figure2_UserResponses_stackedBar+theme(axis.title.y=element_blank(),
...
# save
ggsave("Figures/H2_p_UserJudgements_PAZ_FINAL.pdf",width = 8,height = 4) #width = 7, height = 2,

H2_p_UserJudgements_T=ggarrange(p1_T,p2_T, ncol = 2, nrow = 1, widths=c(6,2.5), common.legend = TRUE, legend="bottom")

# save
ggsave("Figures/H2_p_UserJudgements_T_PAZ_FINAL.pdf",width = 8,height = 4) #width = 7, height = 2,
...
* Is there a significant difference in terms of estimated usefulness for others between groups? We compared number of matches for users in plausible condition (M = `r df_survey_TimingEfficacy_summary$mean[df_survey_TimingEfficacy_summary$group=="P"]`, SEM = `r df_survey_TimingEfficacy_summary$sem[df_survey_TimingEfficacy_summary$group=="P"]`) and users in the closest condition (M = `r df_survey_TimingEfficacy_summary$mean[df_survey_TimingEfficacy_summary$group=="C"]`, SEM = `r df_survey_TimingEfficacy_summary$sem[df_survey_TimingEfficacy_summary$group=="C"]`) using a Wilcoxon–Mann–Whitney U test. This showed: U=`r item10test$statistic `, p=`r item10test$p.value `, r = `r item10effsize `

```{r echo=FALSE, fig.height = 2, fig.width = 5, fig.align = "center"}
# make a H3 plot for publication [combine H3+H4 in speculation about mode of presentation] 
# remove superfluous info before putting it all together
p1=H3_figure2_UserResponses_stackedBar+theme(title=element_blank(),
        axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        panel.spacing = unit(1, "lines"))
...
p1_T=H3_figure2_UserResponses_stackedBar+facet_wrap(vars(itemNo),nrow = 1, ncol = 2,labeller=as_labeller(c(`3`="\"I understood the feedback.\"",`4`="\"I needed support to understand.\"")))+
  theme(panel.spacing = unit(1, "lines"),axis.title.x=element_blank())
p1_T$labels$title=c()

H4_figure2_UserResponses_stackedBar$labels$title=c()
p2=H4_figure2_UserResponses_stackedBar+theme(axis.title.y=element_blank())

p2_T=H4_figure2_UserResponses_stackedBar+theme(title=element_blank(),
...
# save
ggsave("Figures/H3_p_UserJudgements_PAZ_FINAL.pdf",width = 8,height = 4) #width = 7, height = 2,

H3_p_UserJudgements_T=ggarrange(p1_T,p2_T, ncol = 2, nrow = 1, widths=c(6,2.5), common.legend = TRUE, legend="bottom")
# save
ggsave("Figures/H3_p_UserJudgements_T_PAZ_FINAL.pdf",width = 8,height = 4) #width = 7, height = 2,
...
Expl_figure2_UserResponses_stackedBar$labels$title=c()
p3=Expl_figure2_UserResponses_stackedBar+theme(title=element_blank(),
...
# save
ggsave("Figures/H2_H3_expl_survey_PAZ_FINAL.pdf",width = 8,height = 4) #width = 7, height = 2,
...
Expl_figure2_UserResponses_stackedBar$labels$title=c()
p3=Expl_figure2_UserResponses_stackedBar+theme(legend.position = "none",
        panel.spacing = unit(1, "lines"))

leg <- get_legend(Expl_figure2_UserResponses_stackedBar)
...
ggsave("Figures/H2_H3_expl_survey_T_PAZ_FINAL.pdf",width = 8,height = 4) #width = 7, height = 2,
